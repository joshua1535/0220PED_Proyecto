...
void DeleteViajero(){
    if(Vuelos.empty()){
        ...
        do{
            ...
            cin >> n; cin.ignore(); 
            n = n - 1; 
        }while(n < 0 || n >= Vuelos.size());

        string Nombre_Vuelo = Vuelos.at(n).nombre;

        for(int i = 0; i < Vuelos.size(); i++){
            if(Nombre_Vuelo == Vuelos.at(i).nombre){
                queue<persona> Aux_Primera, Aux_Ejecutiva, Aux_Turista;

                if(Vuelos.at(i).Primera.empty() && Vuelos.at(i).Ejecutiva.empty() && Vuelos.at(i).Turista.empty()){
                    cout << "\nEl vuelo [" << Nombre_Vuelo << "] no tiene ning" << char(163) << "n pasajero registrado.\n";
                }
                int AuxID = 0;
                cout << "\nIngrese el DUI del pasajero a eliminar: ";
                cin >> AuxID; cin.ignore();

                while(!Vuelos.at(i).Primera.empty()){
                    if(AuxID == Vuelos.at(i).Primera.front().ID){
                        cout << "\nEliminando al pasajero [" << Aux_Primera.front().name << "del vuelo [" << Nombre_Vuelo << "].\n";
                        Vuelos.at(i).Primera.pop();

                        while(!Aux_Primera.empty()){
                            Vuelos.at(i).Primera.push(Aux_Primera.front());
                            Aux_Primera.pop();
                        }
                    }
                    Aux_Primera.push(Vuelos.at(i).Primera.front());
                    Vuelos.at(i).Primera.pop();
                }
            }
        }
